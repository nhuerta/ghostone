...
void Process_Command( )
{
	...
	if( Command == "/commands" )
	{
		...
		CONSOLE_Print( "   /rcfg               : reload config", false );
		CONSOLE_Print( "", false );
	...
	else if( Command.size( ) >= 7 && Command.substr( 0, 6 ) == "/game " )
	{
		...
		gGProxy->AddSearchGameName(GameName);
	}
...
int main( int argc, char **argv )
{
	...
	if( !CFG.Exists( "war3path" ) || !CFG.Exists( "cdkeyroc" ) || !CFG.Exists( "server" ) || !CFG.Exists( "username" ) || !CFG.Exists( "password" ) || !CFG.Exists( "channel" ) )
	{
		...
		else
		{
			...
			out << "### whispers coming from these will not be shown, ex: pvpgn|realm bot1  //note: use | instead of SPACE" << endl;
			out << "hidewhispersfrom = " << endl;
	...
	else
	{
		...
		HideWhispersFrom = CFG.GetString( "hidewhispersfrom", string());
	...
	CONSOLE_Print( "", false );
	CONSOLE_Print( "  Welcome to GProxyOne.", false );
...
//	wgetch( gInputWindow );
	...
	gGProxy = new CGProxy( Hosts, HideWhispersFrom, UDPBindIP, UDPPort, GUIPort, UDPConsole, PublicGames, FilterGProxy, UDPPassword, UDPTrigger, !CDKeyTFT.empty( ), War3Path, CDKeyROC, CDKeyTFT, Server, Username, Password, Channel, War3Version, Port, EXEVersion, EXEVersionHash, PasswordHashType );
...
CGProxy :: CGProxy( string nHosts, string nHideWhispersFrom, string nUDPBindIP, uint16_t nUDPPort, uint16_t nGUIPort, bool nUDPConsole, bool nPublicGames, bool nFilterGProxy, string nUDPPassword, string nUDPTrigger, bool nTFT, string nWar3Path, string nCDKeyROC, string nCDKeyTFT, string nServer, string nUsername, string nPassword, string nChannel, uint32_t nWar3Version, uint16_t nPort, BYTEARRAY nEXEVersion, BYTEARRAY nEXEVersionHash, string nPasswordHashType )
{
	...
	m_Quit = false;
	UTIL_ExtractStrings(nHideWhispersFrom, m_HideWhispersFrom);
...
//				if( (*i)->GetMapWidth( ) == 1984 && (*i)->GetMapHeight( ) == 1984 )
				uint32_t SlotsOpen = 12;
				uint32_t SlotsTotal = 12;
				if( (*i)->GetMapWidth( ) >= 1984 && (*i)->GetMapHeight( ) >= 1984 
				&&  (*i)->GetMapWidth( ) <= 1996 && (*i)->GetMapHeight( ) <= 1996 )
				{
					...
					if( (*i)->GetMapHeight( ) > 1984 )
					{
						SlotsOpen = (*i)->GetMapWidth( )-1984+1;
						SlotsTotal = (*i)->GetMapHeight( )-1984;
					}
				}

				m_UDPSocket->Broadcast( 6112, m_GameProtocol->SEND_W3GS_GAMEINFO( m_TFT, m_War3Version, MapGameType, MapFlags, MapWidth, MapHeight, GameName, (*i)->GetHostName( ), (*i)->GetElapsedTime( ), (*i)->GetMapPath( ), (*i)->GetMapCRC( ), SlotsTotal, SlotsOpen, m_Port, (*i)->GetUniqueGameID( ), (*i)->GetUniqueGameID( ) ) );
				i++;
			}
...
//		game
		...
		if( game->GetGameName( ) != m_BNET->GetSearchGameName( ) && game->GetGameName( ) != m_BNET->GetSearchGameName2( ) && game->GetReceivedTime( ) < OldestReceivedTime )
			OldestReceivedTime = game->GetReceivedTime( );
	}

//	if( game->GetMapWidth( ) == 1984 && game->GetMapHeight( ) == 1984 )
	if( game->GetMapWidth( ) >= 1984 && game->GetMapHeight( ) >= 1984 
	&&  game->GetMapWidth( ) <= 1996 && game->GetMapHeight( ) <= 1996 )
	{
		GProxyGame = true;
	}
...
void CGProxy :: AddSearchGameName( string GameName)
{
	if( !GameName.empty( ) && GameName.size( ) <= 31 )
	{
		bool s1 = gGProxy->m_BNET->GetSearchGameName().empty();
		bool s2 = gGProxy->m_BNET->GetSearchGameName2().empty();
		string GNN1 = gGProxy->m_BNET->GetSearchGameName();
		string GNN2 = gGProxy->m_BNET->GetSearchGameName2();
		string GN = GetGameName(GameName);
		string GN1 = GetGameName(GNN1);
		string GN2 = GetGameName(GNN2);
		bool alreadySearching = (GameName==GNN1 || GameName==GNN2);
		if (alreadySearching)
		{
			// we're already searching for this but update the time (another 2 minutes) 
			if (GameName==GNN1)
				gGProxy->m_BNET->SetSearchGameName( GameName );
			else
				gGProxy->m_BNET->SetSearchGameName2( GameName );
		} else
			if (s1 || s2)
			{
				if (s1)
			...
			{ 
				...
				if (gGProxy->m_BNET->GetSearchGameNameTime()<gGProxy->m_BNET->GetSearchGameNameTime2())
					gGProxy->m_BNET->SetSearchGameName( GameName );
				else
					gGProxy->m_BNET->SetSearchGameName2( GameName );
			}
...
bool CGProxy :: ShouldHideWhispers(string name)
{
	transform( name.begin( ), name.end( ), name.begin( ), (int(*)(int))tolower );

	string nam;
	for( uint32_t i = 0; i != m_HideWhispersFrom.size(); i++ )
	{
		nam = m_HideWhispersFrom[i];
		UTIL_Replace(nam, "|", "\x20");
		if( nam == name )
		{
			return true;
		}
...
string CGProxy :: GetGameName ( string name)
{
	if (name.empty())
		return string();
	...
	else
	{
		...
		GameName = GameName.substr(0,idx-2);
	}
	return GameName;
}
...
//	m_UDPConsole = CFG->GetInt( "udp_console", 1 ) == 0 ? false : true;
	...
	transform( nHosts.begin( ), nHosts.end( ), nHosts.begin( ), (int(*)(int))tolower );
	UTIL_ExtractStrings(nHosts, m_Hosts);
	string nHideWhispersFrom = CFG->GetString( "hidewhispersfrom", string());
	transform( nHideWhispersFrom.begin( ), nHideWhispersFrom.end( ), nHideWhispersFrom.begin( ), (int(*)(int))tolower );
	UTIL_ExtractStrings(nHideWhispersFrom, m_HideWhispersFrom);
