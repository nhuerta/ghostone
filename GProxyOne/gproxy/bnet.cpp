...
bool CBNET :: Update( void *fd, void *send_fd )
{
	...
	if( m_Socket->GetConnected( ) )
	{
		...
		*/
		...
		if( !m_GProxy->m_LocalSocket && !m_SearchGameName.empty( ) && GetTime( ) - m_LastGetSearchGameTime >= 15 && m_OutPackets.size( ) <= 3 )
		{
			QueueGetGameList( m_SearchGameName );
			m_LastGetSearchGameTime = GetTime( );
		}

		if( !m_SearchGameName2.empty( ) && GetTime( ) - m_SearchGameNameTime2 >= 120 )
		{
			CONSOLE_Print( "[BNET] stopped searching for game \"" + m_SearchGameName2 + "\"" );
			m_SearchGameName2.clear( );
			m_SearchGameNameTime2 = GetTime( );
		}

		if( !m_GProxy->m_LocalSocket && !m_SearchGameName2.empty( ) && GetTime( ) - m_LastGetSearchGameTime2 >= 15 && m_OutPackets.size( ) <= 3 )
		{
			QueueGetGameList( m_SearchGameName2 );
			m_LastGetSearchGameTime2 = GetTime( );
		}
...
void CBNET :: ProcessChatEvent( CIncomingChatEvent *chatEvent )
{
	...
	else if( Event == CBNETProtocol :: EID_WHISPER )
	{
		...
		if (!m_GProxy->ShouldHideWhispers(User))
		if( m_GProxy->m_LocalSocket )
		...
		size_t pos = Message.find("has entered a Warcraft III Frozen Throne game named \"");
		size_t pos2= Message.find("Lobby: ");
		size_t pos3= Message.find("(");
		if (pos != string :: npos || (pos2 != string :: npos && pos3 != string :: npos && pos3>pos2))
		{
			string GameName;
			if (pos != string :: npos)
				GameName = Message.substr(pos+53, Message.length()-pos-53-2);
			else
				GameName = Message.substr(pos2+7, pos3-pos2-7-1);
			m_GProxy->AddSearchGameName(GameName);
			string out = "[GPROXY] Searching for: ";
			string s1 = m_GProxy->m_BNET->GetSearchGameName();
			string s2 = m_GProxy->m_BNET->GetSearchGameName2();
			if (!s1.empty())
				out += "1: ["+s1+"]  ";
			if (!s2.empty())
				out += "2: ["+s2+"]  ";
	...
	else if( Event == CBNETProtocol :: EID_INFO )
	{
		...
		if( m_GProxy->m_LocalSocket )
			m_GProxy->SendLocalChat( "[BNET] " + Message );
...
void CBNET :: SendChatCommand( string chatCommand )
{
	...
	if( m_LoggedIn )
	{
		...
		CONSOLE_Print( "[LOCAL] " + chatCommand );
		string hname = m_GProxy->m_Username;
		if (hname.substr(0,2)=="|c")
			hname = hname.substr(10,hname.length()-10);
		if (m_GProxy->m_Console)
			m_GProxy->UDPChatSend("|Chat "+UTIL_ToString(hname.length())+" " +hname+" "+chatCommand);
...
void CBNET :: QueueChatCommand( string chatCommand )
{
	...
	if( m_LoggedIn )
	{
		...
		bool whisper = (chatCommand.find("/w ") == 0);
		if (whisper && m_PasswordHashType == "pvpgn")
		{
