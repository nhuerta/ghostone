...
//	SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
	...
	{	
        ...
        SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
	SendChat( player, " Ghost One v17.0 r256 - http://ghostone.googlecode.com " );
	SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
	SendChat( player, "     Admin:  " + m_OwnerName );
	SendChat( player, "     Nombre juego:  " + m_GameName );
	if( !m_HCLCommandString.empty( ) )
		SendChat( player, "     HCL Command String:  " + m_HCLCommandString );
	SendChat( player, "     Version:  " + m_GHost->m_Version + " + Mod by LongWang <xet@scene.cl>");
	SendChat( player, "     Usuario:  " + player->GetName( ) );
	SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
