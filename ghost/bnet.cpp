			...
			if( AdminCheck || RootAdminCheck )
			{
				...
				******************/
                ...
                if( Command == "bans" && Payload.empty() )
                {
					...
					string Usr;
					Payload = "SELECT COUNT(*) FROM bans;";
					Usr = Whisper ? User : string( );
					if (m_GHost->m_WhisperAllMessages)
						Usr = User;

					QueueChatCommand("Calculando numero de baneados...", User, Whisper);
...
//										m_GHost->m_AutoHostCountryCheck = false;
				...
				if( ( Command == "delban" || Command == "db" || Command == "unban" || Command == "ub" ) && !Payload.empty( ) )
				{
					...
					if ((m_GHost->m_AdminsLimitedUnban || m_GHost->m_AdminsCantUnbanRootadminBans) && !RootAdminCheck)
					{
						...
						if (m_GHost->m_AdminsLimitedUnban)
...
*/
					...
					if ( VarName.size() >= 3 )
					{
						if ( VarName.substr( 0, 4 ) == "bnet" )
						{
							QueueChatCommand("Battle.net connection settings will not be able to be shown/changed on the fly.", User, Whisper);
							return;
						}
					}
