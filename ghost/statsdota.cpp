				...
				if( ActionData->size( ) >= i + 8 + Data.size( ) )
				{
					...
					if( ActionData->size( ) >= i + 12 + Data.size( ) + Key.size( ) )
					{
						...
						if( DataString == "Data" )
						{
							...
							else if( KeyString.size( ) >= 5 && KeyString.substr( 0, 6 ) == "Assist" )
							{
								// assist 

								string VictimColourString = KeyString.substr( 6 );
								string AssistColourString = KeyString.substr( 7 );
								uint32_t VictimColour = UTIL_ToUInt32( VictimColourString );
								uint32_t AssistColour = UTIL_ToUInt32( AssistColourString );
								CGamePlayer *Killer = m_Game->GetPlayerFromColour( VictimColour );
								CGamePlayer *Victim = m_Game->GetPlayerFromColour( ValueInt );
								CGamePlayer *Assist = m_Game->GetPlayerFromColour( AssistColour );

								CONSOLE_Print( "[STATSDOTA: " + m_Game->GetGameName( ) + "] "+KeyString );

								uint32_t GameNr = m_Game->GetGameNr();

								m_Game->m_GHost->UDPChatSend("|gameupdate "+UTIL_ToString(GameNr));
//								if (Victim)
//									Victim->SetDOTAAssists(Victim->GetDOTAAssists()+1);
								if( Killer && Assist && Victim )
								{
									Assist->SetDOTAAssists(Assist->GetDOTAAssists()+1);
									CONSOLE_Print( "[STATSDOTA: " + m_Game->GetGameName( ) + "] player [" + Assist->GetName( ) + "] assisted in killing player [" + Victim->GetName( ) + "]" );
								}
								else if( Assist )
								{
/*
									if( ValueInt == 0 )
										CONSOLE_Print( "[STATSDOTA: " + m_Game->GetGameName( ) + "] the Sentinel assisted in killing player [" + Victim->GetName( ) + "]" );
									else if( ValueInt == 6 )
										CONSOLE_Print( "[STATSDOTA: " + m_Game->GetGameName( ) + "] the Scourge assisted in killing player [" + Victim->GetName( ) + "]" );
*/
								}
							}
